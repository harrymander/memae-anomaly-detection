...
with torch.no_grad():
    for ite_vid in range(video_num):
        ...
        video_data_loader = DataLoader(
            video_dataset, batch_size=batch_size_in, shuffle=False
        )
        # testing results on a single video sequence
        print(
            "[vidx %02d/%d] [vname %s]" % (ite_vid + 1, video_num, video_name)
        )
        recon_error_list = []
        for batch_idx, (item, frames) in enumerate(video_data_loader):
            ...
            if opt.ModelName == "AE":
                ...
                recon_np = utils.vframes2imgs(
                    unorm_trans(recon_frames.data), step=1, batch_idx=0
                )
                input_np = utils.vframes2imgs(
                    unorm_trans(frames.data), step=1, batch_idx=0
                )
                r = utils.crop_image(
                    recon_np, img_crop_size
            ...
            #     r = (
            #         utils.crop_image(recon_np, img_crop_size)
            #         - utils.crop_image(input_np, img_crop_size)
            #     )
        ...
        np.save(
            os.path.join(te_res_path, video_name + ".npy"), recon_error_list
        )
