...
class VideoDataset(Dataset):
    def __init__(self, idx_root, frame_root, use_cuda=False, transform=None):
        ...
        self.video_list = [
            name
            for name in os.listdir(self.idx_root)
        ...
        for ite_vid in range(len(self.video_list)):
            ...
            idx_file_name_list = [
                name
                for name in os.listdir(os.path.join(self.idx_root, video_name))
                if os.path.isfile(
                    os.path.join(self.idx_root, video_name, name)
                )
            ]
            idx_file_name_list.sort()
            # idx file path list
            idx_file_list = [
                self.idx_root + "/" + video_name + "/" + file_name
                for file_name in idx_file_name_list
            ]
            # merger lists
    ...
    def __getitem__(self, item):
        ...
        tmp_frame = io.imread(
            os.path.join(v_dir, ("%03d" % frame_idx[0]) + ".jpg")
        )
...
class VideoDatasetOneDir(Dataset):
    def __init__(
        ...
        self.idx_name_list = [
            name
            for name in os.listdir(self.idx_dir)
